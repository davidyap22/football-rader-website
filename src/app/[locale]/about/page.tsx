...
const translations: Record<string, Record<string, string>> = {
  ...
  JA: {
    ...
    aboutTitle: "OddsFlowについて：AIサッカー予測とスポーツベッティング分析プラットフォーム",
    aboutSubtitle: "ほとんどの人は試合を見る。少数の人はベッティング市場を見る。",
    aboutIntro: "OddsFlowは後者のために存在します。私たちは「公開統計」（ポゼッション、枠内シュート）と「市場の現実」（資金の流れ、オッズ変動、スマートマネー）の間に大きなギャップがあることに気づきました。このサッカー予測プラットフォームはそのギャップを埋めるために構築しました。私たちは誇大宣伝、直感、「保証された勝利」を扱いません。データ、論理、市場の透明性を扱います。",
    ourStory: "私たちのストーリー",
    ...
    whatWeDoIntro: "ノイズを取り除いてシグナルを見つけます。無駄なトリビアで圧倒するのではなく、スポーツベッティングで本当に重要なメトリクスに焦点を当てています：",
    ...
    philosophyText: "私たちはゲームを尊重しすぎて、あなたに財産を約束することはできません。OddsFlowはあなたのスポーツベッティング武器庫のための武器であり、あなたの脳の代わりではありません。構造化されたデータ、可視化、そして「なぜ」を提供します。事実に基づいた予測の意思決定ができるように、感情ではなく。テクノロジーが私たちの仕事をサポートしますが、人間の判断が決定を導きます。",
    ...
    realityText: "OddsFlowはスポーツベッティングのためのデータ分析プラットフォームです。私たちはブックメーカーでもなく、「保証付きチップサービス」でもありません。地図を渡しますが、運転はあなたがしなければなりません。責任を持ってデータを使用し、ご自身で調査を行い、サッカーでは何も100%確実ではないことを忘れないでください。",
    ...
    footerDesc: "よりスマートなサッカー予測のためのAI駆動オッズ分析。スポーツベッティングをリアルタイムの洞察とデータ駆動の意思決定で。",
    ...
    disclaimer: "免責事項：OddsFlowはAI駆動のサッカー予測を情報および娯楽目的のみで提供しています。予測の精度は保証されず、財務損失について責任を負いません。ギャンブルにはリスクが伴います。責任を持ってお楽しみください。",
  },
  KO: {
